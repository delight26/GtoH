		...
		<div class="row">
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					<c:forEach var="free" items="${frbList }">
						<tr>
							...
							<td class="bb tc"><fmt:formatDate
										value="${free.frbWriteDate }" pattern="yyyy-MM-dd" /></td>
							...
							<td><c:set var="frbEmail" value="${free.frbEmail}" /> <c:set
									...
									<a href="freeupdate?frbNo=${free.frbNo }" data-toggle="modal"
				...
				<c:if test="${ startPage > PAGE_GROUP }">
					<ul class="pager">
						<li><a href="FreeBoardList?pageNum=${ startPage - PAGE_GROUP }">[이전]</a></li>
					</ul>
				</c:if>
				<div class="text-center">
					<ul class="pagination">
						<c:forEach var="i" begin="${ startPage }" end="${ endPage }">
							...
							<c:if test="${ i != currentPage }">
								<li><a href="FreeBoardList?pageNum=${ i }">${ i }</a></li>
							</c:if>
						</c:forEach>
					</ul>
				</div>
				<c:if test="${ endPage < pageCount }">
					<ul class="pager">
						<li><a href="FreeBoardList?pageNum=${ startPage + PAGE_GROUP }">[다음]</a></li>
					</ul>
				</c:if>
			</div>
		</div>
